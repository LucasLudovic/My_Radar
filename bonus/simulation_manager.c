...
void init_shape(manager_t *sim_manager)
{
    ...
    sfRectangleShape_setOrigin(sim_manager->hitbox,
        (sfVector2f){ SPRITE_SIZE / 2, SPRITE_SIZE / 2 });
}

static
void init_sprites(manager_t *sim_manager)
{
    ...
    if (sim_manager->plane_sprite != NULL) {
        sfSprite_setOrigin(sim_manager->plane_sprite,
            (sfVector2f){ SPRITE_SIZE / SCALE / 2, SPRITE_SIZE / SCALE / 2 });
    }
}
